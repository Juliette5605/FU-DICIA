    ...
    switch (_step) {
      ...
      case 3:
        return _PhotoStep(
            ...
            onSkip: _needsPhoto ? null : () => _onPhotoCaptured(null),
...
class _GpsStepState extends State<_GpsStep> {
  ...
  Future<void> _check() async {
    ...
    // VALIDATION GPS AVANCÉE (Victor) - utilise vrai clientId
    final validation = await GpsService.validateLocation(widget.clientId, pos);

    if (mounted) {
      setState(() {
        _loading = false;
        _status = validation.reason;
        _valide = validation.isValid;
      });

      // Si hors zone mais peut être justifié : afficher dialog
      if (!validation.isValid && validation.requiresJustification) {
        // Obtenir le nom du client (TODO: implémenter getClient dans LocalDB)
        final clientNom = 'Client ${widget.clientId}';

        final justification = await showDialog<String>(
          context: context,
          barrierDismissible: false, // force justification
          builder: (context) => GpsJustificationDialog(
            clientNom: clientNom,
            distance: validation.distance,
            onJustified: (justification) =>
                Navigator.of(context).pop(justification),
          ),
        );

        if (justification != null && justification.isNotEmpty) {
          // Justification acceptée : marquer comme valide
          setState(() {
            _valide = true;
            _status = 'Justifié: $justification';
          });
        } else {
          // Pas de justification : reste invalide
          setState(() {
            _valide = false;
            _status = 'Justification requise';
          });
        }
      }
    }
      ...
      child: Column(children: [
        ...
        if (!_loading && !_valide) ...[
          ...
          Container(
            ...
            child: const Text(
                'Vous êtes hors de la zone assignée à ce client. Continuez quand même ?',
        ...
        if (!_loading)
          PrimaryButton(
            label: _valide ? 'CONTINUER' : 'CONTINUER QUAND MÊME',
            color: _valide
                ? const Color(AppColors.green)
                : const Color(AppColors.orange),
